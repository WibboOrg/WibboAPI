...
{   
    ...
    public function getClassement(Request $request, Response $response, array $args): Response
    {
        ...
        $point = User::orderBy('vip_points', 'DESC')->where('user.last_online', '>', time() - $this->minimalTime)->where('user.is_banned', '0')->limit(20)->select('id', 'username', 'look', 'vip_points')->get();
