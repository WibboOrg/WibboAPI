...
class RegisterController extends DefaultController
{
    public function post($request, $response)
    {
        ...
        $limiteip = User::where('ip_last', getUserIP())->count();
        if ($limiteip > 100) {
        ...
        $limiteip = User::where('ip_last', getUserIP())->where('account_created', '>=', $timecreated)->count();
        if ($limiteip > 1) {
        ...
        $id = User::insertGetId([
            ...
            'password' => hashMdp($data->password),
            'rank' => 1,
            'gender' => 'M',
            'motto' => '',
            'credits' => 1000000,
